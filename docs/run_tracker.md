...
Matches `planned_8x8_runs.md`: Config_6 baseline (runs 3–5), then **two** co-evolution settings (30%, 50%) with 3 replicates each, then **two** mutation settings (0.10, 0.20) with 3 replicates each.
...
| 15    | Mutation 0.20 (Condition D) replicate 1 | Complete | 2026-03-16 | 2026-03-18 | evolution_8x8_20260318_051106.txt | 3021           |
| 16    | Mutation 0.20 replicate 2               | Complete | 2026-03-18 | 2026-03-20 | evolution_8x8_20260320_104123.txt | 3022           |
| 17    | Mutation 0.20 replicate 3               | Complete | 2026-03-20 | 2026-03-22 | evolution_8x8_20260322_094426.txt | 3023           |
...
- **Test (5 seeds × 30 games)**: 67/150 → 44.7% [95% CI 36.7%–52.6%], variance across seeds 0.050, train–test gap 0.403
- **Notes**: Large train–test gap: high training win rate vs ~44.7% on the evolution-matched suite.
...
- **Test (5 seeds × 30 games)**: 109/150 → 72.7% [95% CI 65.5%–79.8%], variance across seeds 0.068, train–test gap 0.098
- **Notes**: Better train–test alignment than Run 1; solid evolution-matched test performance.
...
- **Test (5 seeds × 30 games)**: 117/150 → 78.0% [95% CI 71.4%–84.6%], variance across seeds 0.034, train–test gap 0.095
- **Notes**: First replication of Run 2. Evolution-matched test **78.0%** vs Run 2 **72.7%** — improvement over Run 2 on this protocol.
...
- **Test (5 seeds × 30 games)**: 124/150 → 82.7% [95% CI 76.6%–88.7%], variance across seeds 0.025, train–test gap 0.098
- **Notes**: Second replication. Strong evolution-matched test (**82.7%**) before Run 5; good generalization.
...
- **In-file test (single-seed protocol in result file)**: 18/20 wins (90.0%) — legacy line from the run log; use the line below for cross-run comparison.
- **Test (5 seeds × 30 games)**: 142/150 → 94.7% [95% CI 91.1%–98.3%], variance across seeds 0.034, train–test gap 0.003
- **Notes**: Third replication. **Highest evolution-matched test win rate in this tracker (94.7%).** Excellent training–test alignment. (Earlier figures from `reevaluate_strategies.py` were misleading here because move selection differed from evolution.)
...
- **Training Fitness**: 0.0750 (92.5% win rate)
- **Test (5 seeds × 30 games)**: 114/150 → 76.0% [95% CI 69.2%–82.8%], variance across seeds 0.061, train–test gap 0.165
...
- **Notes**: Third replicate of Condition B. **81.3%** evolution-matched test (tied with Run 16 among runs 6–17; Run 5 remains highest overall at 94.7%).
...
- **Notes**: First replicate of Condition C (lower mutation). Very strong test performance (78.0%), slightly below Runs 11 and 16 (81.3%) but above most other conditions.
...
### Run 15 (Condition D: Mutation 0.20) replicate 1 - COMPLETE

- **Date Started**: 2026-03-16
- **Date Completed**: 2026-03-18 05:11:06
- **Configuration**: Condition D — mutation 0.20; Population 400, Gen 75, Games 20, Co-evol 40%, Complexity 0.20. Evolution seed: 3021
- **Result File**: evolution_8x8_20260318_051106.txt
- **Best Strategy**: `add(x[19], min_(x[54], if_(greater_than_or_equal(-10.0, add(abs_(x[68]), x[21])), 0.5, x[24])))`
- **Training Fitness**: 0.1000 (90.0% win rate)
- **Test (5 seeds × 30 games)**: 117/150 → 78.0% [95% CI 71.4%–84.6%], variance across seeds 0.034
- **Notes**: First replicate of Condition D.

### Run 16 (Condition D: Mutation 0.20) replicate 2 - COMPLETE

- **Date Started**: 2026-03-18
- **Date Completed**: 2026-03-20 10:41:23
- **Configuration**: Condition D — mutation 0.20; Population 400, Gen 75, Games 20, Co-evol 40%, Complexity 0.20. Evolution seed: 3022
- **Result File**: evolution_8x8_20260320_104123.txt
- **Best Strategy**: `sub(x[19], sub(x[35], x[24]))`
- **Training Fitness**: 0.0750 (92.5% win rate)
- **Test (5 seeds × 30 games)**: 122/150 → 81.3% [95% CI 75.1%–87.6%], variance across seeds 0.050
- **Notes**: Second replicate of Condition D. **81.3%** evolution-matched test (tied with Run 11 among runs 6–17; Run 5 highest overall at 94.7%).

### Run 17 (Condition D: Mutation 0.20) replicate 3 - COMPLETE

- **Date Started**: 2026-03-20
- **Date Completed**: 2026-03-22 09:44:26
- **Configuration**: Condition D — mutation 0.20; Population 400, Gen 75, Games 20, Co-evol 40%, Complexity 0.20. Evolution seed: 3023
- **Result File**: evolution_8x8_20260322_094426.txt
- **Best Strategy**: `sub(if_(greater_than_or_equal(x[21], max_(-1.0, 0.1)), -0.1, -2.0), x[15])`
- **Training Fitness**: 0.1000 (90.0% win rate)
- **Test (5 seeds × 30 games)**: 103/150 → 68.7% [95% CI 61.2%–76.1%], variance across seeds 0.072
- **Notes**: Third replicate of Condition D. All planned runs (1–17) are now complete.
